# 開発タイムライン

## 2024

### 2024-10

- 2024-10-22: GeoLLaVA, Efficient Fine-Tuned Vision-Language Models for Temporal Change Detection in Remote Sensing
  - https://github.com/HosamGen/GeoLLaVA

### 2024-09

- 2024-09-06: SpatialAnalysisAgent (implementation of GIS Copilot, plugin of QGIS)
  - https://github.com/Teakinboyewa/SpatialAnalysisAgent
  - https://plugins.qgis.org/plugins/SpatialAnalysisAgent-master/

### 2024-08

- 2024-08-23: geospatial-code-llms-dataset, Evaluation of Code LLMs on Geospatial Code Generation
  - https://github.com/kraina-ai/geospatial-code-llms-dataset
- 2024-08-21: smileGeo, Swarm Intelligence in Geo-localization
  - https://anonymous.4open.science/r/ViusalGeoLocalization-F8F5
- 2024-08-18: Geo-FuB, A Method for Constructing an Operator-Function Knowledge Base for Geospatial Code Generation Tasks Using Large Language Models
  - https://github.com/whuhsy/Geo-FuB
- 2024-08-04: AutonomousGIS_GeodataRetrieverAgent (implementation of Autonomous GIS, plugin of QGIS)
  - https://github.com/Teakinboyewa/AutonomousGIS_GeodataRetrieverAgent
  - https://plugins.qgis.org/plugins/AutonomousGIS_GeodataRetrieverAgent/

### 2024-07

- 2024-07-12: GeoText-1652: Towards Natural Language-Guided Drones, Benchmark with Spatial Relation Matching
  - https://github.com/MultimodalGeo/GeoText-1652
- 2024-07-12: CityGuessr: City-Level Video Geo-Localization on a Global Scale
  - https://github.com/ParthPK/CityGuessr

### 2024-06

- 2024-06-01: MapGPT, Map-Guided Prompting with Adaptive Path Planning for Vision-and-Language Navigation
  - https://github.com/chen-judge/MapGPT

### 2024-05

- 2024-05-24: PIGEON, Predicting Image Geolocations
  - https://github.com/LukasHaas/PIGEON
- 2024-05-19: GeoReasoner, Geo-localization with Reasoning in Street Views using a Large Vision-Language Mode
  - https://github.com/lingli1996/GeoReasoner
- 2024-05-11: LLM-Find, An autonomous GIS agent framework for geospatial data retrieval
  - https://github.com/gladcolor/LLM-Find
- 2024-05-04: remote-sensing-image-retrieval, Multi-Spectral Remote Sensing Image Retrieval using Geospatial Foundation Models
  - https://github.com/IBM/remote-sensing-image-retrieval

### 2024-04

- 2024-04-19: LLMGeo, Benchmarking Large Language Models on Image Geolocation In-the-wild
  - https://github.com/yeyimilk/LLMGeo

### 2024-03

- 2024-03-29: Geode, A zero-shot geospatial question answering agent with precise spatiotemporal retrieval and explicit reasoning capabilities
  - https://github.com/devashish-gupta/Geode

### 2024-02

- 2024-02-28: GeoChat, Grounded Large Vision-Language Model for Remote Sensing
  - https://github.com/mbzuai-oryx/GeoChat
- 2024-02-21: GeoLLM, Extracting Geospatial Knowledge from Large Language Models and Large Language Models are Geographically Biased
  - https://github.com/rohinmanvi/GeoLLM
  - https://rohinmanvi.github.io/GeoLLM/
- 2024-02-17: VQASynth (3rd party implementation of SpatialVLM)
  - https://github.com/remyxai/VQASynth
- 2024-02-14: ArcGIS Generative AI Assistants
  - https://mediaspace.esri.com/media/t/1_opret32t
  - https://doc.arcgis.com/en/arcgis-online/administer/configure-assistants.htm

### 2024-01

- 0224-01-25: Geo-knowledge-guided-GPT-models-for-disaster-response
  - https://github.com/geoai-lab/Geo-knowledge-guided-GPT-models-for-disaster-response

## 2023

### 2023-12

- 2023-12-13 GeoCLIP, Clip-Inspired Alignment between Locations and Images for Effective Worldwide Geo-localization
  - https://github.com/VicenteVivan/geo-clip
- 2023-12-10: charites-ai
  - https://github.com/yuiseki/charites-ai

### 2023-11

- 2023-11-16: SatCLIP, Global, General-Purpose Location Embeddings with Satellite Imagery
  - https://github.com/microsoft/satclip
- 2023-11-07: Loc2Vec, Learning Location Embeddings with Triplet-loss Networks
  - https://github.com/angus-spence/loc2vec

### 2023-10

- 2023-10-24: CurriculumLoc, Visual Geolocalization via Multi-Stage Refinement
  - https://github.com/npupilab/CurriculumLoc
- 2023-10-22: GeoLM, Empowering Language Models for Geospatially Grounded Language Understanding
  - https://github.com/knowledge-computing/geolm

### 2023-08

- 2023-08-30: OverpassNL (OverpassT5), Text-to-OverpassQL: A Natural Language Interface for Complex Geodata Querying of OpenStreetMap
  - https://github.com/raphael-sch/OverpassNL

### 2023-07

- 2023-07-28: GeoGalactica: A Scientific Large Language Model in Geoscience
  - https://github.com/geobrain-ai/geogalactica
  - https://huggingface.co/geobrain-ai/geogalactica
- 2023-07-16: GeoGPT, an open-source and non-profit exploratory research project for global geoscience research, promotes the open science concept of collaboration, sharing, and co-construction
  - https://geogpt.zero2x.org.cn/
- 2023-07-08: OSM-GPT, project aims to bridge the gap between natural language queries and querying the OpenStreetMap database, making it easy for users to explore and uncover the treasures hidden within
  - https://github.com/rowheat02/osm-gpt

### 2023-06

- 2023-06-30: GeoNet, Benchmarking Unsupervised Adaptation across Geographies
  - https://github.com/ViLab-UCSD/GeoNet
- 2023-06-21: Geo Aware LLM, A Multi-granularity Decade-Long Geo-Tagged Twitter Dataset for Spatial Computing
  - https://github.com/ResponsibleAILab/Geo-Aware-LLM
- 2023-06-14: Geo-SAM (plugin of QGIS), using Segment Anything Model (SAM) to accelerate segmenting or delineating landforms in geospatial raster images
  - https://github.com/coolzhao/Geo-SAM
- 2023-06-05: K2, A Foundation Language Model for Geoscience Knowledge Understanding and Utilization
  - https://github.com/davendw49/k2
- 2023-06-06: GPT4GEO, How a Language Model Sees the World's Geography
  - https://github.com/jonathan-roberts1/GPT4GEO

### 2023-05

- 2023-05-19: llllm,A suite of tools to perform geospatial operations using Large Language Models
  - https://github.com/developmentseed/llllm
- 2023-05-13: TRIDENT
  - https://github.com/yuiseki/TRIDENT
- 2023-05-10: GeoQuestions1089, Crowdsourced Geospatial Question-Answering dataset containing triples of question-queries-answers
  - https://github.com/AI-team-UoA/GeoQuestions1089
  - https://huggingface.co/datasets/AI-team-UoA/GeoQuestions1089
- 2023-05-02: QGPTAgent (plugin of QGIS), that utilizes the advanced natural language processing capabilities of the OpenAI GPT model to automate various processes in QGIS
  - https://github.com/momaabna/QGPTAgent

### 2023-04

- 2023-04-26: Palantir AIP
  - https://www.youtube.com/watch?v=XEM5qz__HOU
- 2023-04-19: samgeo (segment-geospatial), A Python package for segmenting geospatial data with the Segment Anything Model (SAM)
  - https://github.com/opengeos/segment-geospatial
  - https://samgeo.gishub.org/
- 2023-04-17: LLM-Geo (implementation of Autonomous GIS), self-generating, self-organizing, self-verifying, self-executing, and self-growing
  - https://github.com/gladcolor/LLM-Geo

### 2023-03

- 2023-03-30: osm-ai-map, interpret natural language requests into OpenStreetMap Overpass API queries
  - https://github.com/steveattewell/osm-ai-map
- 2023-03-18: ChatGeoPT, a toy Geospatial AI Assistant that translates natural language text into OpenStreetMap Overpass API calls
  - https://github.com/earth-genome/ChatGeoPT
  - https://medium.com/@bengmstrong/chatgeopt-exploring-the-future-of-talking-to-our-maps-b1f82903bb05

### 2023-02

- 2023-02-12: QChatGPT (plugin of QGIS), integration between QGIS and OpenAI API. Displays the results from OpenAI in QGIS.
  - https://github.com/KIOS-Research/QChatGPT

## 2022

### 2022-12

- 2022-12-30: Cloud_Map_Evaluation, MapEval
  - https://github.com/JokerJohn/Cloud_Map_Evaluation

### 2022-11

- 2022-11-09: geo-twitter, Predicting the Geolocation of Tweets Using transformer models on Customized Data
  - https://github.com/K4TEL/geo-twitter

### 2022-10

- 2022-10-22: G^3, Geolocation via Guidebook Grounding
  - https://github.com/g-luo/geolocation_via_guidebook_grounding
- 2022-10-21: SpaBERT, A Pretrained Language Model from Geographic Data for Geo-Entity Representation
  - https://github.com/knowledge-computing/spabert
- 2022-10-13: MGeo, Multi-Modal Geographic Language Model Pre-Training
  - https://github.com/PhantomGrapes/MGeo

## 2021

### 2021-05

- 2021-05-26: Geo-Bench
  - https://github.com/ServiceNow/geo-bench

### 2021-02

- 2021-02-18: mlg_evaldata
  - https://github.com/google-research-datasets/mlg_evaldata
- 2021-02-14: TopoBERT, A Visual Tool for Exploring Word Representations from Fine-Tuned Transformer Models
  - https://github.com/tdavislab/TopoBERT

## 2020

### 2020-05

- 2020-05-07: LwM_SIGSPATIAL2020_ToponymMatching, A Deep Learning Approach to Geographical Candidate Selection through Toponym Matching
  - https://github.com/Living-with-machines/LwM_SIGSPATIAL2020_ToponymMatching

## 2019

### 2019-09

- 2019-09-05: GeoAI2019Geoparser
  - https://github.com/geoai-lab/GeoAI2019Geoparser

### 2019-07

- 2019-07-18: geonorm
  - https://github.com/clulab/geonorm

### 2019-05

- 2019-05-27: EUPEG, An Extensible and Unified Platform for Evaluating Geoparsers
  - https://github.com/geoai-lab/EUPEG
